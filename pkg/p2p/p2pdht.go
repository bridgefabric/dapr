...
func NewP2P(serviceName string) *P2P {
	...
	nodehost, kaddht, _ := setupHost(ctx, []byte(""))
	// Debug log
...
func setupHost(ctx context.Context, privateKey []byte) (host.Host, *dht.IpfsDHT, error) {
	...
	basicConnMgr, err := connmgr.NewConnManager(100, 400, connmgr.WithGracePeriod(time.Minute*10))
	if err != nil {
		logrus.WithFields(logrus.Fields{
			"error": err.Error(),
	...
	opts := libp2p.ChainOptions(identity, listen, security, transport, muxer, conn, nat, routingOpt, relay, libp2p.EnableNATService())
