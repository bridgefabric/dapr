...
func (p *Pool) Get(timeout time.Duration) (wapc.Instance, error) {
	instanceIface, err := p.rb.Poll(timeout)
	if err != nil {
		return nil, fmt.Errorf("get from pool timed out: %w", err)
	}

	inst, ok := instanceIface.(wapc.Instance)
	if !ok {
...
func (p *Pool) Return(inst wapc.Instance) error {
